...
public class CalculosService implements CalcularImposto {
	...
	public double calculaIR(Funcionario funcionario, Integer dep) {
		...
		if (calculoBase < 1903.98) {
			...
			funcionario.setDescontoIR(
					(((funcionario.getSalarioBruto() - (dep * dependenteIR) - funcionario.getDescontoInss()) * 7.5)
							/ 100) - 142.80);
			return funcionario.getDescontoIR();
		} else if (calculoBase >= 2826.66 && calculoBase <= 3751.05) {
			funcionario.setDescontoIR(
					(((funcionario.getSalarioBruto() - (dep * dependenteIR) - funcionario.getDescontoInss()) * 15)
							/ 100) - 354.80);
			return funcionario.getDescontoIR();
		} else if (calculoBase >= 3751.06 && calculoBase <= 4664.68) {
			funcionario.setDescontoIR(
					(((funcionario.getSalarioBruto() - (dep * dependenteIR) - funcionario.getDescontoInss()) * 22.5)
							/ 100) - 636.13);
			return funcionario.getDescontoIR();
		} else {
			funcionario.setDescontoIR(
					(((funcionario.getSalarioBruto() - (dep * dependenteIR) - funcionario.getDescontoInss()) * 27.5)
							/ 100) - 869.36);
			return funcionario.getDescontoIR();
		}
	}
